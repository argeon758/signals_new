			...
			elseif sig.Routes then
				for k,v in pairs(sig.Routes) do
					if v.RouteName and v.RouteName:upper() == comm[1] then
						if sig.LastOpenedRoute and sig.LastOpenedRoute ~= k then sig:CloseRoute(sig.LastOpenedRoute) end
						sig:CloseRoute(k)
					end
				end
			end
...
local function StringSetChar(str, index, char)
...
function ENT:Initialize()
	...
	self.KGUActive = false
    self.KGULenseIndex = 0
end
function ENT:SetKGUOverride(active, lense, arscode)
    self.KGUActive = active
    self.KGU = active -- Для отображения буквы "К" на маршрутнике (в вашем коде это уже есть)
    
    if active then
        self.KGULenseIndex = tonumber(lense) or 0
    else
        self.KGULenseIndex = 0
    end
end
...
function ENT:PostInitalize()
	...
	if self.Left and (self.SignalType != 4 and self.SignalType != 3 and self.SignalType != 6) then
		self:SetModel("models/signals_msa/tunnels/dtm_tunnel_mirror.mdl")
...
function ENT:CheckOccupation()
	if not self.Close then --not self.OverrideTrackOccupied and
		...
		if self.OccupiedByNowOld ~= self.OccupiedByNow then
			...
			for k, v in pairs( ents.FindByClass( "gmod_track_arm" )) do
                local RealOccupied = false
                if IsEntity(self.OccupiedBy) then
                    RealOccupied = true
                end
                v:OnOccupationSig(self.Name,RealOccupied,self.OccupiedBy)
				
                if self.Colors ~= nil then
                    v:UpdateSignal(self.Name,self.Colors)
                end
            end
		end
	else
...
function ENT:ARSLogic(tim)
	...
	if not self.Routes[self.Route or 1].Repeater  then
		...
		if self.Occupied or not self.NextSignalLink or not self.NextSignalLink.FreeBS then
			...
			self.FreeBS = math.min(25,self.NextSignalLink.FreeBS + 1) -- old 10 freebs - костыль
		end
...
function ENT:Think(my)
	...
	if Metrostroi.SignalDebugCV:GetBool() then
		...
		self:SetNW2Int("ArsNext",self.ARSNextSpeedLimit)
	...
	if not self.ControllerLogic then
		...
		if self.MU or self.ARSOnly or (self.RouteNumberSetup and self.RouteNumberSetup ~= "") or (self.RouteNumber and self.RouteNumber ~= "") then
			if self.NextSignalLink then
				...
				self.RouteNumberOverrite = number
			else
...
function ENT:SendUpdate(ply)
	net.Start("metrostroi-signal")
		...
		net.WriteInt(self.SignalType or 0, 4)
		...
		net.WriteString(self.BoxName or "")
		net.WriteString(self.BoxNameStart or "")
		net.WriteBool(self.StationTrack)
